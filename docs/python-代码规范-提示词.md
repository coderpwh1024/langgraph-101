# Python 代码规范提示词（基于 Google Python Style Guide + 本项目约定）

> 用途：作为 AI 编码助手 / 代码评审的系统提示词（system prompt），也可作为团队 Python 代码规范说明。
> 适用项目：`langgraph-101`（LangChain + LangGraph 教学项目）。
> 规范来源：[Google Python Style Guide](https://google.github.io/styleguide/pyguide.html)，并结合本项目实际特点裁剪。
