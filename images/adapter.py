...
def _build_markdown_card_payload(content: str) -> str:
    """Build a card JSON 2.0 payload for markdown content.

    The card v2 ``markdown`` component renders tables, headings and quotes,
    which post-type ``md`` elements cannot. Requires Feishu client >= 7.20.
    """
    card = {
        "schema": "2.0",
        "body": {
...
class FeishuAdapter(BasePlatformAdapter):
            ...
            for chunk in chunks:
                ...
                try:
                    ...
                    if msg_type == "interactive":
                        # 卡片被 API 拒绝时降级为纯文本，保底可达（与旧行为一致）。
                        logger.warning("[Feishu] Card payload rejected by API (%s); falling back to plain text", exc)
                ...
                if (
                    ...
                    logger.warning(
                        "[Feishu] %s payload rejected by API response; falling back to plain text", msg_type
                    )
                    response = await self._feishu_send_with_retry(
                        chat_id=chat_id,
                        msg_type="text",
    ...
    async def edit_message(
        ...
        *,
        ...
        try:
            # 编辑接口（im.v1.message.update）不支持 interactive 卡片，禁用卡片路径。
            msg_type, payload = self._build_outbound_payload(content, allow_card=False)
    ...
    def _build_outbound_payload(self, content: str, *, allow_card: bool = True) -> tuple[str, str]:
        # Feishu post-type 'md' elements do not render markdown tables; sending
        # table content as post causes the message to appear blank on the client.
        # Card JSON 2.0 markdown components render tables properly, so prefer an
        # interactive card. The message-update API does not accept interactive
        # payloads, so edit paths pass allow_card=False and keep the plain-text
        # degradation.
